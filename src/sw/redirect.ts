...
function spaceAt(s: string, i: number): number {
  ...
  if (c === CH_PLUS) return 1;
  if (c === CH_PERCENT && s.charCodeAt(i + 1) === CH_2 && s.charCodeAt(i + 2) === CH_0) return 3;
  return 0;
}
...
function findExcl(s: string, start: number, end: number): [number, number] {
  for (let i = start; i < end; i++) {
    if (s.charCodeAt(i) === CH_EXCL) return [i, 1];
    if (i + 2 < end && isEncodedExclAt(s, i)) return [i, 3];
  }
  return [-1, 0];
}
...
function findLastSpaceExcl(s: string, start: number, end: number): [number, number, number] {
...
function findLastSpace(s: string, start: number, before: number): [number, number] {
  for (let i = before; i >= start; i--) {
    if (s.charCodeAt(i) === CH_PLUS) return [i, 1];
    if (
      s.charCodeAt(i) === CH_PERCENT &&
      i + 2 <= before &&
      s.charCodeAt(i + 1) === CH_2 &&
...
function rawFixup(s: string, from: number, to: number): string {
  ...
  if (plusPos === -1) {
    if (raw.indexOf("%2F") === -1 && raw.indexOf("%2f") === -1) return raw;
    let out = "";
    let seg = 0;
    for (let i = 0; i < raw.length; i++) {
      if (raw.charCodeAt(i) === CH_PERCENT && i + 2 < raw.length && raw.charCodeAt(i + 1) === CH_2) {
  ...
  for (let i = seg; i < raw.length; i++) {
    ...
    if (c === CH_PLUS) {
      ...
      seg = i + 1;
    } else if (hasSlash && c === CH_PERCENT && i + 2 < raw.length && raw.charCodeAt(i + 1) === CH_2) {
      const c2 = raw.charCodeAt(i + 2);
      if (c2 === 70 || c2 === 102) {
        out += raw.substring(seg, i) + "/";
        seg = i + 3;
...
function fillTemplate(url: string, s: string, termStart: number, termEnd: number): string {
...
function resolveRaw(
  ...
  let end = len;
  while (end > start) {
    if (rawQuery.charCodeAt(end - 1) === CH_PLUS) { end--; continue; }
    if (
      end >= start + 3 &&
  ...
  if (c0 === CH_BSLASH && end - start > 1) {
    ...
    if (luckyUrl) return [fillTemplate(luckyUrl, rawQuery, termStart, end), null];
  ...
  if (lastChar === CH_EXCL) {
    ...
    if (rawQuery.charCodeAt(end - 2) === CH_PLUS) {
      const termEnd = end - 2;
      if (termEnd <= start) return ["/", null];
      if (luckyUrl) return [fillTemplate(luckyUrl, rawQuery, start, termEnd), null];
  ...
  if (
    ...
    isEncodedExclAt(rawQuery, end - 3)
  ) {
  ...
  if (afterE < end) {
    ...
    if (spAfter) {
      ...
      if (url) {
        ...
        if (termStart >= end) return [originOf(url), bang];
  ...
  if (afterE >= end || (lastChar === CH_EXCL && afterE === end)) {
    ...
    if (hasSpace === -1) {
      const bang = rawQuery.substring(start, exclPos).toLowerCase();
      const url = custom[bang] || BANGS[bang];
      if (url) return [originOf(url), bang];
      return [fillTemplate(defaultUrl, rawQuery, start, end), null];
    }
  }

  // "cats+!g"
  const [spExclPos, spExclLen, seWidth] = findLastSpaceExcl(rawQuery, start, end);
  if (spExclPos !== -1) {
    ...
    if (bangStart < end) {
      ...
      if (bangStr.indexOf("+") === -1 && !bangStr.includes("%20")) {
        ...
        const url = custom[bang] || BANGS[bang];
        if (url) return [fillTemplate(url, rawQuery, start, spExclPos), bang];
        return [fillTemplate(defaultUrl, rawQuery, start, end), null];
      }
    }
  }
  ...
  // "cats+g!"
  if (lastChar === CH_EXCL || (end >= 3 && isEncodedExclAt(rawQuery, end - eWidth))) {
    const bangExclEnd = lastChar === CH_EXCL ? end - 1 : end - 3;
    const [lastSpPos, lastSpLen] = findLastSpace(rawQuery, start, bangExclEnd - 1);
    if (lastSpPos !== -1) {
      ...
      if (bangStart2 < bangExclEnd) {
        ...
        const url = custom[bang] || BANGS[bang];
        if (url) return [fillTemplate(url, rawQuery, start, lastSpPos), bang];
        return [fillTemplate(defaultUrl, rawQuery, start, end), null];
      }
    }
  }
